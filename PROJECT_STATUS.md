...
- **Email auth**: SPF (`include:resend.com include:amazonses.com`) + DKIM (Resend) + DMARC (`p=quarantine`) + BIMI (logo SVG, no VMC)
...
| Google Search Console | SEO monitoring (OAuth credentials in .env.local) |
| Ahrefs | Site audit + analytics (`ECL265EZNoHAwvuR19Pwaw`) |
| Cloudflare | DNS management (Zone ID: `51e953748e8f48ccfa35e5616c3c0e4c`) |
...
- [2026-03-15] **Security audit fixes**: patchata vulnerabilità `flatted` DoS (npm audit fix), rimosso `re_dummy_key` fallback in 5 API routes
- [2026-03-15] **Cart recovery traduzioni complete**: tutte le 24+ chiavi recovery tradotte in 10 lingue (FR/DE/ES/PT/PL/RU/UK/AR aggiunte), ChangeHero UI labels localizzate, fix spacing step + indirizzo USDT bold
- [2026-03-15] **BIMI + Email authentication**: SPF (`include:resend.com include:amazonses.com`), DMARC (`p=quarantine`), BIMI logo SVG Tiny P/S (gold "A" su navy) — logo visibile su Apple Mail, Yahoo, Fastmail
...
- **DMARC upgrade** — intorno a 2026-04-01, cambiare `p=quarantine` → `p=reject` se nessun problema email
